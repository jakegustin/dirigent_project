## Azure 500 on Dirigent - Modified
...
- On your local machine run `./scripts/set_cpu_freq.sh modified [Num_Fast_Nodes] user@node0 user@node1 ... user@nodeN`
  - For example, if you want 7 worker nodes as fast nodes, you should do `./scripts/set_cpu_freq.sh modified 7 user@node0 user@node1 ... user@nodeN`
...
- Copy traces from this folder on local machine to `node0` using `scp artifact_evaluation/azure_500/modified/azure_500/* user@node0:~/invitro/data/traces/azure_500/`, replacing `user@node0` with what's in your CloudLab experiment.
